...
async def registrar_servicio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mecanico_nombre:
        cur.execute("SELECT id FROM usuarios WHERE LOWER(nombre) = LOWER(%s) AND activo = TRUE", (mecanico_nombre,))
        row = cur.fetchone()
        if row:
            mecanico_id = row[0]
    ...
    """, (num, COLORES.get(color, color), monto, pendiente, descripcion, mecanico_id, usuario["id"]))
...
async def registrar_pago_pendiente(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cur.execute("UPDATE servicios SET monto_pendiente = %s, estado = %s WHERE id = %s",
                (nuevo_pendiente, "pagado" if nuevo_pendiente == 0 else "activo", servicio_id))
    ...
    if nuevo_pendiente == 0:
        await update.message.reply_text(f"✅ Deuda de *{num} {color_nombre}* saldada completamente", parse_mode="Markdown")
    else:
        await update.message.reply_text(f"✅ Pago de ${pagado:.2f} registrado. Pendiente: *${nuevo_pendiente:.2f}*", parse_mode="Markdown")
